...
class DiamondShape extends LogoShape {
  // The constructor function initializes properties for the logo text, text color, background color, and logo shape.
  // It sets the logo shape to 'diamond' and passes the other properties to the parent constructor.
  constructor({ logoText, textColor, bgColor }) {
    super({ logoText, textColor, bgColor, logoShape: 'diamond' });
  }

  // The render function returns an SVG string representation of a diamond shape.
  render() {
    return `<polygon points="50,5 95,50 50,95 5,50" fill="${this.bgColor}" stroke="${this.textColor}" stroke-width="5" />`;
  }
}

// This exports all the classes as an object.

module.exports = { LogoShape, CircleShape, SquareShape, TriangleShape, DiamondShape };
